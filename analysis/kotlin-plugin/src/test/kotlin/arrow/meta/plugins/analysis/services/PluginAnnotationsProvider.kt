package arrow.meta.plugins.analysis.services
...
class PluginAnnotationsProvider(testServices: TestServices) : EnvironmentConfigurator(testServices) {
    companion object {
        private const val ANNOTATIONS_JAR_DIR = "../laws/build/libs/"
        private val ANNOTATIONS_JAR_FILTER = FilenameFilter { _, name -> name.startsWith("arrow-analysis-laws") && name.endsWith(".jar") }
    }
    ...
    private val failMessage = { "Jar with annotations does not exist. Please run :arrow-analysis-laws:jvmJar" }
}
